    ...
    def postprocess(self, pred, proto):
        ...
        for i, det in enumerate(pred):
            ...
            for onemask in masks:
                ...
                # newmaski = newmaski.repeat(3, axis=2)
                newmaski = newmaski[:,:,0]
   
                # cv2.imwrite("onemask.jpg", onemask)
                # cv2.imwrite("newmaski.jpg", newmaski)
...
if __name__ == "__main__":
    ...
    model = StoneSeg(modelpath, className=["stone"], size=640, conf_thres=0.2, iou_thres=0.45)
    model.modelinit()
    ...
    savefolder = r"/root/project/Modules/Demo/seg/draws"
    if not os.path.exists(savefolder):
        os.mkdir(savefolder)
        
    set_split_size = (512, 512)
    imagepath = r"/root/project/Modules/Demo/seg/A_4542.JPG"
    img = cv2.imread(imagepath)
    h,w = img.shape[:2]
